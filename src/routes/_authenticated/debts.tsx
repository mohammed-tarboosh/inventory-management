...
function Page() {
  ...
  const exportXlsx = () => exportToExcel(statement.map((r) => ({
    date: r.transaction_date, customer: customerName(r.customer_id),
    type: r.transaction_type, amount: r.amount, currency: r.currency_code,
    amount_local: r.amount_local, balance: r.balance, ref: r.invoice_ref, notes: r.notes,
  })), "customer_statement");

  const exportPdf = () => exportTablePDF({
    title: t("customer_statement_report"),
    meta: customerId !== "_" ? [`${t("customers")}: ${customerName(customerId)}`] : [],
    head: [t("date"), t("customers"), t("transaction_type"), t("amount"), t("currency"), t("price_local"), t("balance"), t("invoice_ref")],
    body: statement.map((r) => [fmtDate(r.transaction_date), customerName(r.customer_id), r.transaction_type, r.amount, r.currency_code, r.amount_local, r.balance, r.invoice_ref ?? "-"]),
  ...
  return (
    <div>
      <PageHeader title={t("debts")}>
        ...
        {can("debts.manage") && <TxForm customers={customers as any[]} currencies={currencies as any[]} defaultCustomer={customerId !== "_" ? customerId : undefined} onDone={refetch} />}
      </PageHeader>
      <div className="p-4 bg-card border rounded-md mb-4 flex flex-wrap gap-4 items-end">
        ...
        <div className="text-sm space-y-1">
          <div>{t("total_debit")}: <strong>{fmtNum(totals.d, 2)}</strong></div>
          <div>{t("total_credit")}: <strong>{fmtNum(totals.c, 2)}</strong></div>
          <div>{t("balance")}: <strong>{fmtNum(totals.bal, 2)}</strong></div>
        </div>
      </div>
      <DataTable
        rows={statement}
        columns={[
          ...
          { key: "t", header: t("transaction_type"), cell: (r: any) => r.transaction_type === "debit" ? t("debit") : t("credit_d") },
          { key: "a", header: t("amount"), cell: (r: any) => `${fmtNum(r.amount, 2)} ${r.currency_code}` },
...
function TxForm({ customers, currencies, defaultCustomer, onDone }: { customers: any[]; currencies: any[]; defaultCustomer?: string; onDone: () => void }) {
  const { t } = useI18n();
  ...
  const submit = async () => {
    if (!customer_id || !amount) { toast.error("invalid"); return; }
    const { data: u } = await supabase.auth.getUser();
    const { error } = await supabase.from("debt_transactions").insert({
      customer_id, transaction_type, amount, currency_code, exchange_rate,
      amount_local: amount * exchange_rate,
      transaction_date, invoice_ref: invoice_ref || null, notes: notes || null,
