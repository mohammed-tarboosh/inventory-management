...
function CurrenciesTab() {
  ...
  return (
    <div className="space-y-3">
      {can("settings.manage") && <CurrencyForm onDone={refetch} />}
...
function CurrencyForm({ onDone }: { onDone: () => void }) {
  const { t } = useI18n();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>{t("cancel")}</Button><Button onClick={submit} disabled={!code || !ar}>{t("save")}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
...
function RatesTab() {
  ...
  return (
    <div className="space-y-3">
      {can("settings.manage") && <RateForm currencies={currencies as any[]} onDone={refetch} />}
      <DataTable rows={rows as any[]} columns={[
...
function RateForm({ currencies, onDone }: { currencies: any[]; onDone: () => void }) {
  const { t } = useI18n();
  const [open, setOpen] = useState(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button><Plus className="h-4 w-4 me-1" />{t("add")}</Button></DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>{t("exchange_rates")}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div>
