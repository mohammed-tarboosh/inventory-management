...
function SForm({ row, onDone }: { row?: any; onDone: () => void }) {
  ...
  const [default_payment_type, setDefaultPaymentType] = useState(
    row?.default_payment_type ?? "cash",
  );
  ...
  const submit = async () => {
    ...
    if (row) payload.updated_by = u.user?.id;
    else payload.created_by = u.user?.id;
    const { error } = row
