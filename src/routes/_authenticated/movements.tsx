...
function Page() {
  ...
  const { data: movements = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from("stock_movements").select("*").order("movement_date", { ascending: false });
      ...
      let rows = data ?? [];
      if (catId !== "_") {
        const ids = new Set((items as any[]).filter((i) => i.category_id === catId).map((i) => i.id));
  ...
  const exportXlsx = () => exportToExcel(
    (movements as any[]).map((m) => ({
      date: m.movement_date, item: itemName(m.item_id), type: m.movement_type, qty: m.quantity, price: m.unit_price_local,
    })),
    "movements"
  );
  const exportPdf = () => exportTablePDF({
    ...
    body: (movements as any[]).map((m) => [fmtDate(m.movement_date), itemName(m.item_id), m.movement_type, m.quantity, m.unit_price_local]),
    filename: "movements",
  });
        ...
        <div>
          ...
          <Select value={itemId} onValueChange={setItemId}>
            ...
            <SelectContent>
              ...
              {(items as any[]).map((i) => <SelectItem key={i.id} value={i.id}>{i.name_ar}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        <div>
          ...
          <Select value={catId} onValueChange={setCatId}>
            ...
            <SelectContent>
              <SelectItem value="_">{t("all")}</SelectItem>
              {(categories as any[]).map((c) => <SelectItem key={c.id} value={c.id}>{c.name_ar}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
      </div>
